...
config_list = {
    # id: [map_name, randomize_each_reset, max_steps, gamma, epsilon, dec_epsilon, learning_rate, description]
...
for cfg_id, cfg in config_list.items():
    map_name, randomize, max_steps, gamma, epsilon, dec_epsilon, lr, desc = cfg

    print("\n" + "=" * 80)
    print(f"Config {cfg_id}")
    print(f"Mapa: {map_name} | Randomize: {randomize}")
    print(f"Gamma: {gamma}")
    ...
    env = gym.make(
        ...
        randomize_each_reset=randomize,
        seed=123,
    )
