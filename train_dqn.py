...
from algos.DQNAgent import DQNAgent
...
for cfg_id, cfg in config_list.items():
    ...
    agent = DQNAgent(
        ...
        min_epsilon=0.05,
        checkpoint_dir=f"model/dqn/",
        name=f"dqn_config_{cfg_id}",
    )
    ...
    plot_learning_curve(f"logs/episodes_logs_dqn_{map_name}_{cfg_id}.csv")
